...
def factor_testing(check_factor_df, group_column):
    ...
    # 计算年化波动率
    annualized_volatility = long_short_returns.std() * np.sqrt(12)
    ...
    results = pd.DataFrame({
        'Metric': ['IC Mean', 'IC Std', 'Information Ratio', 'Rank IC Mean', 'Rank IC Std', 'Rank Information Ratio', 'Annualized Return', 'Annualized Volatility', 'Max Drawdown', 'Monthly Win Rate'],
        'Value': [ic_mean, ic_std, ir, rank_ic_mean, rank_ic_std, rank_ir, annualized_return, annualized_volatility, max_drawdown, monthly_win_rate]
    })
